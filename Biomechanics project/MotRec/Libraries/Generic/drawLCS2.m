function drawLCS2(BodyData, varargin)
...
%      BodyData.ALNames:      cell with AL names
%      BodyData.ALCoords:     array (3 x nALs) with AL local coordinates
...
%      AntebrazoData.ALNames      = {'US', 'RS', 'EJC', 'WJC'};
%      AntebrazoData.ALCoords     = [Antebrazo_Pos_US, Antebrazo_Pos_RS, Antebrazo_Pos_EJC, Antebrazo_Pos_WJC];
...
ALNames       = BodyData.ALNames; 
MarkerNames   = BodyData.MarkerNames; 
ALCoords      = BodyData.ALCoords; 
...
% find scale
MaxMakersCoord = max(max(abs(MarkerCoords)));
MaxALCoord     = max(max(abs(ALCoords)));
MaxCoord       = max([MaxMakersCoord MaxALCoord]);
...
% Plot calculated Markers referred to LCS
nALs = size(ALCoords,2);
for i = 1 : nALs
    plot3(ALCoords(1,i), ALCoords(2,i), ALCoords(3,i), 'ro');
    text(ALCoords(1,i), ALCoords(2,i), ALCoords(3,i), ['  ',ALNames{i}], 'FontSize', 10, 'Color', 'r');
    line([0, ALCoords(1,i)], [0,ALCoords(2,i)], [0,ALCoords(3,i)], 'Color', 'r');
end
